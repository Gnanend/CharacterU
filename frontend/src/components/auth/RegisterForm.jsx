  ...
  return <div className="w-full max-w-md mx-auto bg-dark-900 border border-dark-800 rounded-2xl shadow-xl overflow-hidden">
      <div className="p-8 space-y-6">
        <div className="text-center space-y-2">
          <h2 className="text-2xl font-bold text-white tracking-tight">{t('createAnAccount', 'Create an Account')}</h2>
          <p className="text-slate-400 text-sm">{t('joinCharacterUAnd', 'Join CharacterU and start creating')}</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-1">
            <label className="text-sm font-medium text-slate-300">{t('fullName', 'Full Name')}</label>
            <input type="text" name="fullName" value={formData.fullName} onChange={handleChange} className={`w-full px-4 py-3 bg-dark-950 border ${validationErrors.fullName ? 'border-red-500 focus:ring-red-500' : 'border-dark-800 focus:ring-primary-500'} rounded-xl text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:border-transparent transition-all`} placeholder={t('johnDoe', 'John Doe')} disabled={loading} />
          ...
          {/* Role Dropdown */}
          <div className="space-y-1">
            <label className="text-sm font-medium text-slate-300">{t('iAmA', 'I am a...')}</label>
            <select name="role" value={formData.role} onChange={handleChange} className="w-full px-4 py-3 bg-dark-950 border border-dark-800 rounded-xl text-white focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent transition-all appearance-none" disabled={loading}>
              <option value="student">{t('studentPlayer', 'Student / Player')}</option>
              <option value="employer">{t('studioEmployer', 'Studio / Employer')}</option>
              <option value="family">{t('familyParent', 'Family / Parent')}</option>
            </select>
          </div>
