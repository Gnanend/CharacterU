...
const Topbar = ({
  ...
  return <header className="h-20 bg-dark-950/80 backdrop-blur-md border-b border-dark-800 sticky top-0 z-30 flex items-center justify-between px-6 lg:px-10">
      
      {/* Mobile Menu Trigger & Search */}
      <div className="flex items-center gap-4">
        <button onClick={onOpenSidebar} className="md:hidden p-2 text-slate-400 hover:text-white rounded-lg hover:bg-dark-800 transition-colors" aria-label="Open sidebar">
        ...
        <div className="hidden lg:flex items-center gap-2 bg-dark-900 border border-dark-800 rounded-xl px-4 py-2.5 w-80 focus-within:ring-2 focus-within:border-transparent focus-within:ring-primary-500/50 transition-all group">
          ...
          <input type="text" placeholder={t('searchYourProgress', 'Search your progress...')} className="bg-transparent border-none outline-none text-sm text-white placeholder-slate-500 w-full" />
        </div>
      </div>
